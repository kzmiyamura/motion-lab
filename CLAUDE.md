...
| `src/hooks/usePoseEstimation.ts` | MediaPipe Pose 骨格推定・パターン検出・シーケンス生成・ML推論フック |
| `src/components/FilePlayer.tsx` | ローカル動画プレイヤー（骨格オーバーレイ・シーケンスビュー・ML制御含む） |
| `src/components/AnnotationTool.tsx` | ポーズアノテーションツール・ML学習UI |
...
| `src/engine/poseClassifier.ts` | TF.js ブラウザ内学習・推論エンジン（SHR + 相対位置特徴量） |
| `src/engine/googleAuth.ts` | Google OAuth2 トークン管理（localStorage キャッシュ） |
| `src/engine/googleDrive.ts` | Google Drive API（ファイル一覧・アップロード・ダウンロード） |
...
Pass 1 で検出した人物の「頭部のみ」を楕円でマスクして Pass 2 を実行する（旧: 胴体全体の矩形 fillRect）。

**変更理由**: CBL など手前を通過するケースで、旧矩形マスクが手前の人物の胴体・肩まで覆い、Pass 2 での再検出を妨げていた。顔周辺のみをマスクすることで手前の人物の骨格を視野に残す。

**楕円サイズ（優先順位）**:
1. 肩幅 (lm[11]〜[12]) が取れる → `rx = sw×0.35, ry = sw×0.45`
2. 耳幅 (lm[7]〜[8]) が取れる   → `rx = ew×2.0,  ry = ew×2.5`
3. フォールバック               → `rx=0.08, ry=0.10`（画像幅の 8%）

**デバッグ可視化**: 骨格 ON 中（`vizMode !== 'off'`）に楕円境界線をゴールド破線でキャンバスに描画。`ellipseMasksRef` に Pass 2 で使用した楕円情報を保持。
...
- **CBL_FIX（肩幅クロス照合 → nose フォールバック）**: `justSeparated` 直後の si0/si1 逆転を2段階で検出・修正
  - **一次判定（肩幅クロス照合）**: 現フレームの3D肩幅（XZ平面 hypot）とプロファイル平均3D肩幅の誤差合計を direct/swapped で比較。swapped < direct × 0.80 なら強制スワップ
    - 最小肩幅閾値 0.04（正規化）未満は計測不安定とみなし除外
    - 発動: `[CBL_FIX_SW]` ログ / スキップ: `[CBL_FIX_SW_SKIP]` ログ
  - **フォールバック（nose アンカー）**: 肩幅が取れない場合のみ、slot.nose（旧顔位置）との距離比較
    - 発動: `[CBL_FIX_NOSE]` ログ / スキップ: `[CBL_FIX_NOSE_SKIP]` ログ
  - **両方不可**: `[CBL_FIX_SKIP]` ログ（肩幅不足 + nose 欠損）
...
ブラウザ内 TF.js でリーダー/フォロワーを分類するオプション機能。ルールベース SHR 判定の補完として動作。

### 特徴量（FEATURE_SIZE=13）— `poseClassifier.ts: extractFeatures()`

| 特徴量 | 説明 |
|--------|------|
| `ps0`, `ps1` | 各人の SHR（3D肩幅/3D腰幅）。`Math.hypot(x,z)` で横向き時も正確計測 |
| `ps0-ps1` | SHR の差 |
| `bodyH0`, `bodyH1` | 身長（鼻→腰中点Y距離） |
| `bodyH0-bodyH1` | 身長差 |
| `hipX0`, `hipX1` | 腰中点 X 位置 |
| `hipX0-hipX1` | 腰位置差 |
| `sw0`, `sw1` | 3D肩幅 |
| `hw0`, `hw1` | 3D腰幅 |

### 学習フロー（AnnotationTool.tsx）

- **累積学習方式**: Drive の全 `salsa_annotated_v1_*` ファイルを一括取得し、全フレームを合算して1回の `trainModel()` を呼ぶ。per-file の学習履歴管理（旧 `TRAINED_KEY`）は廃止
- **ラベルとリーダー特定**:
  - `standard_pos` → `poses[0]` = Leader（定義による）
  - `side_L_right` → hipX が大きい方 = Leader
  - `side_L_left` → hipX が小さい方 = Leader
  - `overlap` / `complex_turn` / `single` / `ignore` → スキップ
- **Data Augmentation**: 各フレームで leader-first と follower-first の2サンプルを生成（クラス均衡）
- **モデルアーキテクチャ**: Dense(32,relu) → Dropout(0.2) → Dense(16,relu) → Dense(2,softmax)
- **モデル保存先**: IndexedDB（`indexeddb://salsa-role-model-v2`）+ Google Drive（`salsa_role_model_v2.json`）の両方に保存

### ⚠️ ML がルールベースと差別化できない根本原因

**現在のラベル方式の問題**: `side_L_right`「右の人がリーダー」、`side_L_left`「左の人がリーダー」というラベルは**位置からリーダーを決めている**。V2特徴量（関節座標＋速度）を使っても「どちらが右か左か」を学習するだけになり、ルールベースの SHR 判定と本質的に同じ結論になる。

**ML が真に差別化できる条件**:
- ラベルの根拠を「位置」ではなく「**この動画でこの人物がリーダーである（事前に人間が確認済み）**」にする
- ルールベースが失敗するシーンのデータが必要:
  - CBL 通過中（お互いが交差する瞬間）
  - 横向き・背向き（肩幅が潰れる）
  - 2人の体格が近い（ps 差が 0.05 以下）
  - オクルージョン中（片方が隠れている）

**現在取れるアクション**:
1. アノテーション時に「この動画全体で左の人がリーダー」という前提で `standard_pos` ラベルを使う（位置に依存しない）
2. ルールベースが色を間違えている瞬間のフレームを重点的にラベル付けする
3. 将来的には「動画単位でリーダーを指定する」UI を追加し、全フレームに人物IDをそのまま正解として使う

### 推論アーキテクチャ

- **呼び出し方**: `FilePlayer` が `mlModel` と `mlTf`（TF.js モジュール）を `usePoseEstimation` に渡す
- **推論タイミング**: RAFループ内で同期的に実行（`predictLeaderProbSync`）。React state を経由しない
  - 旧設計（React state 経由）は `setMlResult()` の非同期性により実際には毎フレーム「次フレームへ」になってしまい、色変更が適用されなかった
  - **修正**: 推論を RAFループ内に移動し、`personRoles.set()` で描画直前にロールを同期上書き
- **表示**: ML ON 中は canvas 右上に `🤖 L=S{n} {%}` バッジ表示。`mlResult: { leaderSlot, prob }` を `usePoseEstimation` が返す

### `onRawPoses` のスロット順序

`onRawPoses` コールバックは MediaPipe の生検出順（`all[0]`, `all[1]`）ではなく、**スロット割り当て後（`slots[0].detectedIdx = si0` 確定後）にスロット順で渡す**。
これにより `AnnotationTool` 側が受け取る `poses[0]` が常に `slots[0]`（Left/Leader側）と対応する。

### 既知の制限

- **ML ≈ ルールベース（現状）**: ラベル付けが位置ベース（左右どちらか）に依存しているため、V2 特徴量（関節座標＋速度）を使っても SHR ルールと同じ決定境界を学習してしまう。上記「⚠️ ML がルールベースと差別化できない根本原因」を参照。
- **将来の改善方向**: 「動画単位でリーダーを人間が指定」する UI を作り、全フレームに正解IDを付与する方式にすることで、位置に依存しない真の学習データが得られる
