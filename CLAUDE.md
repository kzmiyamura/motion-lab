...
- **iOS パス（メインスレッド）**:
  ...
  - バッテリー節約のため描画は `detectInterval` 間隔のみ（RAF 毎フレームは非実行）
- **PC パス（Web Worker）**:
  - `src/workers/salsaAnalyzer.worker.ts` を `new Worker(new URL(...), { type: 'module' })` で起動
  - メインスレッドで `createImageBitmap(video)` → Worker に Transferable として転送（ゼロコピー）
  - Worker 内で 2パスカスケードを実行（OffscreenCanvas でマスク描画）
  - Worker からランドマーク配列を `postMessage` → `latestLandmarksRef.current` に保存
  - メインスレッドは毎 RAF フレーム `latestLandmarksRef.current` で描画（UI スレッド解放）
  - Worker 失敗時は `workerFailedRef.current = true`（フレームが更新されなくなるのみ）
- **共通**: `isPoseCoherent()` による合成人体フィルタは両パスで適用
- **Vite 設定**: `vite.config.ts` に `worker: { format: 'es' }` を追加（IIFE はコード分割ビルド非対応）

## アーキテクチャ制約メモ

- **OffscreenCanvas + WebWorker（iOS）非対応**: iOS Safari は Worker 内 WebGL コンテキスト（GPU delegate）が利用不可。そのため iOS は常にメインスレッドで処理し、スロー再生時のみ 2パスカスケードを実行する
- **PC Worker の MediaPipe 初期化**: Worker 内でも CDN（jsdelivr）から WASM をフェッチしてロード。`workerReadyRef.current` が true になるまで（数秒）はフレームが届かないため初期描画は空になる
