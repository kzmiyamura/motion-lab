...
- **モデル保存先**: IndexedDB（`indexeddb://salsa-role-model`）+ Google Drive の両方に保存
...
- **ML ≈ ルールベース**: 現状の13特徴量はすべて SHR・身長・腰位置という物理比率であり、ルールベース SHR 判定と本質的に同じ情報を使う。ルールベースが正しいケースで学習したデータが多いため、ML もほぼ同じ判定結果になる
- **改善案**: CBL 遷移・背向き等のルールベースが失敗するフレームを重点的にアノテーションする、または時系列特徴量（速度・加速度）を追加することで差別化可能
