...
### 1.3 運用前提（2026-07-29 時点）

- **個人利用・アップロード頻度は少ない**（1日数本以下）。よって:
  - レート制限に実際に当たる可能性は低い → バックオフリトライは実装するが簡素でよい（P0 では単純リトライで十分。凝るのは実際に当たってから）
- ただし「フレーム数比例の禁止」（§1.1）は**本数が少なくても維持**する。1本が長い動画は全フレーム方式だと1本で枠を使い切るため（30fps・1分 ≈ 画像1,800枚 ≈ 数百万トークン相当）。本数の多寡とは無関係の制約

### 1.4 段階的エスカレーション方針（確定）

**まず JS/CV の計算に頑張らせ、Claude の利用は最小から始める。CV の品質が足りないと判明した箇所だけ、少しずつ Claude の負荷を上げる。**

- レベル0（初期値）: Claude はレポート生成＋contested 区間の裁定のみ。キーフレームは最小限
- レベル1: 特定項目（例: 技名ラベリング）で CV 単独の品質が不足 → その項目だけキーフレーム数・判断範囲を増やす
- レベル2: それでも不足 → 当該区間のフレームサンプリング密度を上げる（それでもフレーム数比例にはしない）
- 逆方向も可: Claude 裁定の正解パターンが溜まったら CV 側のルール/閾値に還元し、Claude の出番を減らす
