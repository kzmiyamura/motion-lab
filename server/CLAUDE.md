...
### 変更内容（git pull で入る）
1. **Turn 検出**: 肩幅（身長比で正規化）が直近3秒の中央値の半分未満に落ちる状態が0.3秒続いたら回転とみなす。外見IDで「リーダー/フォロワーのどちらが回ったか」も特定
2. **CBL 検出**: 2人の腰X座標の交差（左右の入れ替わり）。交差前後2秒以内に十分な分離があるものだけ採用（密着ジッタを除外）
3. 出力: `measurements.json` の `summary.events[]` = `{t, type: "Turn"|"CBL", by: "leader"|"follower"|"pair"}`。同種イベントは2.5秒のクールダウン
4. デバッグ動画に検出瞬間の黄色ラベル（`TURN (follower)` 等）を1.2秒焼き込み
5. runner-prompt: Claude が events 候補を検分し、明らかな誤検出を落としてレポートの「技のタイムライン」表に載せる
6. サルサ（ペア）フォルダの指示書を version 2 に更新済み（Mac側からリモートで実施済み — ThinkCentre側の作業不要）

### やってほしいこと
1. `git pull` && `pm2 restart motion-lab-server`
2. 同じ動画（2fda2815）で再解析し、以下を報告:
   - `summary.events` の件数と内訳（Mac実測: **10件 = CBL 6 + Turn 4**。Turn の内訳は follower 1 / leader 3）
...
- Turn の担い手（leader/follower）は外見IDに依存するため、回転中に背面になると稀に取り違え得る。Claude 裁定で吸収する設計
