...
### 【追記】再現可能な技記述への拡張（同じく git pull で入る）

「0:12 ターン」のような出来事ログではなく**読んだダンサーが再現できる記述**をレポートに書かせる方針になった（PCの持ち主の指示）。

- `summary.events[]` に **`rotations`（回転数、ダブルターン対応）と `hold`（手のつなぎ候補: 「リーダー右手×フォロワー左手」等）** を追加
- jobWorker が**技イベント毎のキーフレーム**（`<秒>_turn.jpg` / `<秒>_cbl.jpg`、上限12件）を書き出すようになった。Claude はこの画像から回転方向・手のつなぎ・入り/出口を読み取り、技ごとに再現可能な記述を書く
