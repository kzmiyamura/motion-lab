...
- `out/measurements.json` — CV（YOLOv8-pose）による計測結果。数値の正はこちら
  ...
  - `summary.events[]` — 技イベントの**候補**:
    ...
    - `spin` は回る向きの目安: `seq` は真横を通過するたびの向き（R=右回り・L=左回り・?=不明）、`netDeg` はその合計角度
      （360 = 右回り1回、-360 = 左回り1回、0 = 半回転して戻っただけの可能性）。10fps の計測なので ±180° ずれることがあり、
      人手校正では向きが女性のターン12件中10件で正しかった。**最終判断はストリップ画像で行う**
...
- `out/keyframes/*.jpg` — 静止画
  ...
  - `*_turn_strip.jpg` / `*_cbl_strip.jpg` — 各技イベントの**ストリップ**: イベント時刻の 0.4秒前〜1.6秒後を
    0.1秒刻みで並べた一覧画像（各コマ左上に時刻）。ファイル名の数字がイベント時刻。
