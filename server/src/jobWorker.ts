...
async function runJob(job: AnalysisJobRow): Promise<void> {
  ...
  try {
    ...
    // 技イベント毎に、0.1秒刻みの連続フレームを時刻付きで並べた一覧画像（ストリップ）を1枚書き出す。
    // 旧方式の「入り・最中・出口」の3枚（0.6秒間隔）では回転の向き・回転数・手のつなぎを読み違えていた
    // （9/23 の人手校正で実測）。上限超過時は均等サンプリング（終盤の技が丸ごと消える実測バグの修正）
...
const MAX_EVENT_KEYFRAMES = 24; // 一覧画像（ストリップ）を書き出す技イベント数の上限（1件1枚。1分程度の動画なら全候補を覆う）
