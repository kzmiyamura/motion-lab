 ...
 *
 * 2026-08-17（ユーザー指示「男がすこし上体を斜めにすれば背中に手も届きやすい。
 * 男女間の距離も斜めの分が広くなる。男性が女性の足を踏まないように」）:
 * 腰の間隔を 0.37 → **0.4532m** へ広げた（半分なので 0.185 → 0.2266）。
 * ユーザーの俯瞰図（円＝胴体、線＝顔の向き）の実測とも一致する:
 * ベーシック 100px = 0.70m のとき、クローズドは 64px = **0.45m**。
 * 届かないぶんは CoupleFigure 側で男の上体を 40° 振って補う
 * （右肩が 0.18×sin40° = 11.6cm 前へ出る）。足は骨盤の子なので動かず、
 * 広げた 8.3cm がそのまま**足どうしの余裕**になる。
 */
const CLOSED_HALF_GAP = 0.2266;
